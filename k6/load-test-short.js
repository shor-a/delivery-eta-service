/**
 * Delivery ETA Service — Short Load Test
 * ========================================
 * Condensed version of the full test for quick iteration.
 * Get a fast read on service behaviour before committing to the full test
 * Total duration: ~2 minutes.
 *
 * Stages:
 *   1. warmup    (30s, 10 VUs)       — minimal JIT warm-up
 *   2. ramp-up   (30s, 10 → 50 VUs)  — find the climb
 *   3. sustained (30s, 50 VUs)       — short steady-state window
 *   4. spike     (30s, 100 VUs)      — burst ceiling
 *
 * Run (with Grafana):
 *   podman run --rm -i `
 *     --network eta-net `
 ...
 *   podman run --rm -i --network eta-net -v "${PWD}/k6:/scripts" grafana/k6 run /scripts/load-test-short.js
 */
...
export function setup() {
  ...
  console.log("Health check passed — starting short load test");
}

export function teardown() {
  console.log(
    "Short load test complete. Check Grafana at http://localhost:3000",
  );
}
